  ...
  useEffect(() => {
    ...
    const process = async () => {
      ...
      try {
        if (isEsewa) {
          ...
          if (isSuccessPath) {
            ...
            if (paymentStatus.paymentStatus !== "COMPLETED") {
              setErrorMessage(paymentStatus.gatewayResponseMessage ?? "Payment was not completed in eSewa.");
              setStatus("failure");
              return;
            }
          } else {
            ...
            setErrorMessage(paymentStatus.gatewayResponseMessage ?? "Payment was cancelled or failed in eSewa.");
            setStatus("failure");
            return;
          }
        } else {
          ...
          if (paymentStatus.paymentStatus !== "COMPLETED") {
            setErrorMessage(paymentStatus.gatewayResponseMessage ?? "Payment was not completed in Khalti.");
            setStatus("failure");
            return;
          }
        }
        ...
        setErrorMessage(getApiErrorMessage(err, "Payment verification failed."));
        setStatus("failure");
      }
    };
