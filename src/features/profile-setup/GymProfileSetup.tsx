...
const HEADERS: Record<GymStepId, [string, string, string]> = {
  gymInfo: ["Basic Gym", "Information", "Gym name, type, registration number and capacity."],
...
const FitPalGymSetup: FC = () => {
  ...
  const addDoc = () => {
    ...
    setDocs((prev) => [...prev, { type: next?.value ?? "OTHER", fileName: "", uploaded: false }]);
  };

  const removeDoc = (idx: number) => {
    if (docs[idx]?.type === "REGISTRATION_CERTIFICATE" || docs[idx]?.type === "LICENSE") return;
  ...
  const markDocUploaded = (idx: number, name: string) => {
    setDocs((prev) => prev.map((doc, index) => (index === idx ? { ...doc, uploaded: true, fileName: name } : doc)));
  };

  const clearDoc = (idx: number) => {
    setDocs((prev) => prev.map((doc, index) => (index === idx ? { ...doc, uploaded: false, fileName: "" } : doc)));
  };
    ...
    <div className="screen">
      ...
      <div className="field">
        ...
        <div className="pill-group">
          {GYM_TYPES.map((type) => (
            <div key={type} className={`pill${gymType === type ? " sel" : ""}`} onClick={() => setGymType((current) => current === type ? null : type)}>
              {type}
            </div>
          ))}
        </div>
      ...
      <div className="frow">
        <div className="field">
          <label>Registration No. <span style={{ color: "#ef4444" }}>*</span></label>
          <input type="text" placeholder="e.g. 123456/079/080" value={gymRegNo} onChange={(event) => setGymRegNo(event.target.value)} />
        </div>
        <div className="field">
          <label>Established Year <span style={{ color: "#ef4444" }}>*</span></label>
          <input type="number" placeholder="e.g. 2018" min={1900} max={2026} value={gymEstablished} onChange={(event) => setGymEstablished(event.target.value)} />
        </div>
      </div>

      <div className="field">
        <label>Maximum Member Capacity <span style={{ color: "#ef4444" }}>*</span></label>
        <input type="number" placeholder="e.g. 150" min={1} value={gymCapacity} onChange={(event) => setGymCapacity(event.target.value)} />
        <div className="field-hint">Total concurrent members your facility can safely accommodate.</div>
      </div>

      <Actions label="Save and Continue" step={step} totalSteps={STEPS.length} hideBack onBack={() => undefined} onNext={() => setStep(1)} />
    </div>
  );
  ...
  const ScreenLocation: FC = () => (
    <div className="screen">
        ...
        <div style={{ display: "flex", flexDirection: "column", background: "var(--muted)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 16, overflow: "hidden" }}>
          <div style={{ padding: "18px 18px 16px", borderBottom: "1px solid rgba(255,255,255,.05)", display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <button className="av-btn" type="button">Upload Logo</button>
            </div>
          </div>
      ...
      <Actions label="Save and Continue" step={step} totalSteps={STEPS.length} onBack={() => setStep(0)} onNext={() => setStep(2)} />
    </div>
  );
  ...
  const ScreenDocs: FC = () => {
    ...
    return (
      <div className="screen">
            ...
            <div key={`${doc.type}-${idx}`} style={{ background: "var(--muted)", border: `1px solid ${isRequired ? "rgba(249,115,22,.15)" : "rgba(255,255,255,.06)"}`, borderRadius: 14, padding: "14px 16px", marginBottom: 12, position: "relative" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ flex: 1, position: "relative" }}>
                  <select
                    ...
                    style={{ width: "100%", padding: "11px 32px 11px 14px", background: isRequired ? "rgba(249,115,22,.06)" : "rgba(255,255,255,.04)", border: `1px solid ${isRequired ? "rgba(249,115,22,.2)" : "rgba(255,255,255,.08)"}`, borderRadius: 10, color: isRequired ? "var(--orange)" : "#e5e7eb", fontFamily: "var(--font)", fontSize: 13, fontWeight: 600, outline: "none", appearance: "none", WebkitAppearance: "none", cursor: isRequired ? "default" : "pointer" }}
                  >
                    {isRequired ? (
              ...
              {doc.uploaded ? (
                ...
                <div
                  ...
                  onClick={() => markDocUploaded(idx, `doc_${idx + 1}.pdf`)}
                  onKeyDown={() => undefined}
                  role="button"
                  tabIndex={0}
                ...
                >
                  ...
                  <span style={{ fontSize: 12, fontWeight: 500, color: "#4b5563" }}>Click to upload</span>
        ...
        <Actions label="Submit for Review" step={step} totalSteps={STEPS.length} onBack={() => setStep(1)} onNext={() => setStep(3)} />
      </div>
    );
  };
  ...
  const SCREEN_MAP: Record<GymStepId, FC> = {
    gymInfo: ScreenGymInfo,
    location: ScreenLocation,
    docs: ScreenDocs,
    gymDone: ScreenGymDone,
  };
  ...
  const Screen = SCREEN_MAP[stepId];
