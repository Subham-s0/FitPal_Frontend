...
import type { RecommendationMode } from "@/features/gyms/types";
...
const RecommendationModeBar = ({
  ...
  const visibleModes = showBestMatch
    ? modes
    : modes.filter(({ id }) => id !== "best-match");

  return (
    <div
      className={`grid w-full gap-1.5 rounded-[20px] border border-white/10 bg-[rgba(10,10,10,0.92)] p-1.5 shadow-2xl backdrop-blur-xl md:max-w-none ${
        showBestMatch
          ? "max-w-[380px] grid-cols-3"
          : "max-w-[260px] grid-cols-2"
      }`}
    >
      {visibleModes.map(({ id, label, icon: Icon, requiresLocation }) => {
