...
import { getAdminUsersApi } from "@/features/admin/admin-user.api";
import {
  approveGymAnnouncementApi,
...
import {
  ...
  type AdminAnnouncementEditorValue,
  type AnnouncementTargetOption,
} from "@/features/announcements/components/AnnouncementEditorDialog";
import { RejectAnnouncementDialog } from "@/features/announcements/components/RejectAnnouncementDialog";
...
const DEFAULT_EDITOR: AdminAnnouncementEditorValue = {
  title: "",
  content: "",
  audienceScope: "ALL_USERS",
  visibilityScope: "IN_APP",
  scheduledAt: "",
  expiresAt: "",
  selectedTargets: [],
};
...
type AdminAnnouncementSortMode = "NEWEST" | "OLDEST" | "UPDATED" | "TITLE_A_Z" | "TITLE_Z_A";

const SORT_OPTIONS: { value: AdminAnnouncementSortMode; label: string }[] = [
...
const canEdit = (announcement: AnnouncementSummaryResponse) =>
  announcement.sourceType === "ADMIN" &&
  announcement.publishStatus !== "PUBLISHED" &&
  announcement.publishStatus !== "EXPIRED";

const canPublish = (announcement: AnnouncementSummaryResponse) =>
  announcement.sourceType === "ADMIN" &&
  announcement.publishStatus !== "PUBLISHED" &&
  announcement.publishStatus !== "EXPIRED";

const canDelete = (announcement: AnnouncementSummaryResponse) =>
  announcement.sourceType === "ADMIN" &&
  announcement.publishStatus !== "PUBLISHED" &&
  announcement.publishStatus !== "EXPIRED";

const toTargetOptions = (detail: AnnouncementDetailResponse): AnnouncementTargetOption[] =>
  detail.targetAccounts.map((target) => ({
    accountId: target.accountId,
    displayName: target.displayName,
    secondaryLine: target.secondaryLine,
    role: target.role,
  }));

const toEditorValue = (detail?: AnnouncementDetailResponse | null): AdminAnnouncementEditorValue =>
  detail
    ? {
        title: detail.title,
        content: detail.content,
        audienceScope: detail.audienceScope,
        ...
        selectedTargets: toTargetOptions(detail),
      }
    : DEFAULT_EDITOR;

const COL_W = ["34%", "7%", "11%", "11%", "12%", "11%", "11%", "3%"];

export default function AdminAnnouncementsPage() {
  const [query, setQuery] = useState("");
  const [sourceType, setSourceType] = useState<AnnouncementSourceType | undefined>(undefined);
  ...
  const [sort, setSort] = useState<AdminAnnouncementSortMode>("NEWEST");
  ...
  const [editorValue, setEditorValue] = useState<AdminAnnouncementEditorValue>(DEFAULT_EDITOR);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [detailView, setDetailView] = useState<AnnouncementDetailResponse | null>(null);
  const [rejectTarget, setRejectTarget] = useState<AnnouncementSummaryResponse | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<AnnouncementSummaryResponse | null>(null);
  const [rejectionReason, setRejectionReason] = useState("");
  const [targetSearch, setTargetSearch] = useState("");
  const [targetRoleFilter, setTargetRoleFilter] = useState<"USER" | "GYM" | undefined>(undefined);
  ...
  const listQuery = useQuery({
    queryKey: ["admin-announcements", query, sourceType, reviewStatus, publishStatus, sort, page],
    queryFn: () =>
      getAdminAnnouncementsApi({
  ...
  const targetQuery = useQuery({
    queryKey: ["admin-announcement-target-search", targetSearch, targetRoleFilter],
    queryFn: () =>
      getAdminUsersApi({
        query: targetSearch.trim() || undefined,
        role: targetRoleFilter,
        page: 0,
        size: 8,
      }),
    enabled: editorOpen && editorValue.audienceScope === "SPECIFIC_ACCOUNTS",
    staleTime: 30_000,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: AdminAnnouncementUpsertRequest) =>
      editingId ? updateAdminAnnouncementApi(editingId, payload) : createAdminAnnouncementApi(payload),
    onSuccess: async () => {
      await invalidateAll();
      setEditorOpen(false);
      setEditingId(null);
  ...
  const publishMutation = useMutation({
    mutationFn: publishAdminAnnouncementApi,
    onSuccess: async () => {
      await invalidateAll();
      toast.success("Announcement publish state updated");
    },
    onError: (error) => toast.error(getApiErrorMessage(error, "Failed to publish announcement")),
  });
  ...
  const cancelMutation = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, "Failed to cancel announcement")),
  });

  const approveMutation = useMutation({
    mutationFn: approveGymAnnouncementApi,
    onSuccess: async () => {
      await invalidateAll();
      toast.success("Gym announcement approved");
    },
    onError: (error) => toast.error(getApiErrorMessage(error, "Failed to approve announcement")),
  });

  const rejectMutation = useMutation({
    mutationFn: ({ announcementId, reason }: { announcementId: number; reason: string }) =>
      rejectGymAnnouncementApi(announcementId, reason),
    onSuccess: async () => {
      await invalidateAll();
      setRejectTarget(null);
      setRejectionReason("");
      toast.success("Gym announcement rejected");
    },
    onError: (error) => toast.error(getApiErrorMessage(error, "Failed to reject announcement")),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, "Failed to delete announcement")),
  });

  const targetOptions = useMemo(
    () =>
      (targetQuery.data?.items ?? [])
        .map<AnnouncementTargetOption>((item) => ({
          accountId: item.accountId,
          displayName: item.displayName,
          secondaryLine: item.secondaryLine || item.email,
          role: item.role,
        }))
        .filter((item) => !editorValue.selectedTargets.some((s) => s.accountId === item.accountId)),
    [editorValue.selectedTargets, targetQuery.data?.items]
  );
  ...
  const activeFilterCount =
    Number(sourceType != null) + Number(reviewStatus != null) + Number(publishStatus != null);
  ...
  const openCreate = () => {
    ...
    setEditorValue(DEFAULT_EDITOR);
    setTargetSearch("");
    setTargetRoleFilter(undefined);
    setEditorOpen(true);
  };

  const loadDetail = async (announcementId: number) =>
    queryClient.fetchQuery({
      queryKey: ["admin-announcement-detail", announcementId],
      queryFn: () => getAdminAnnouncementApi(announcementId),
    });
  ...
  const submitEditor = () => {
    saveMutation.mutate({
      ...
      audienceScope: editorValue.audienceScope,
      visibilityScope: editorValue.visibilityScope,
      scheduledAt: toIsoOrNull(editorValue.scheduledAt),
      expiresAt: toIsoOrNull(editorValue.expiresAt),
      targetAccountIds:
        editorValue.audienceScope === "SPECIFIC_ACCOUNTS"
          ? editorValue.selectedTargets.map((t) => t.accountId)
          : undefined,
    });
  };
    ...
    <div className="dashboard-mobile-page space-y-5 font-['Outfit',system-ui,sans-serif]">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-[32px] font-black tracking-tight text-white">
            Admin <span style={fireStyle}>Announcements</span>
          </h1>
        </div>
        ...
        {statsQuery.isLoading ? (
          ...
          <>
            ...
            <div className="flex min-h-[102px] flex-col rounded-2xl border border-white/15 bg-white/[0.03] p-3.5">
              <div className="flex items-center gap-2">
                <Users className="h-4 w-4 flex-shrink-0 table-text-muted" />
                <p className="text-[10px] font-black uppercase tracking-wider table-text-muted">Admin broadcasts</p>
              </div>
              <div className="mt-2.5 flex min-h-[28px] items-center">
                <p className="text-2xl font-black leading-none table-text">{stats?.adminAnnouncementCount ?? 0}</p>
              </div>
              <p className="mt-1 text-[10px] table-text-muted">{stats?.gymAnnouncementCount ?? 0} gym submissions</p>
            </div>
            <div className="flex min-h-[102px] flex-col rounded-2xl border border-white/15 bg-white/[0.03] p-3.5">
              <div className="flex items-center gap-2">
                <CalendarClock className="h-4 w-4 flex-shrink-0 table-text-muted" />
                <p className="text-[10px] font-black uppercase tracking-wider table-text-muted">Scheduled</p>
              </div>
              <div className="mt-2.5 flex min-h-[28px] items-center">
                <p className="text-2xl font-black leading-none table-text">{stats?.scheduledCount ?? 0}</p>
              </div>
              <p className="mt-1 text-[10px] table-text-muted">{stats?.publishedCount ?? 0} already live</p>
            </div>
            <div className="flex min-h-[102px] flex-col rounded-2xl border border-red-500/25 bg-red-500/[0.06] p-3.5">
              <div className="flex items-center gap-2">
                <RadioTower className="h-4 w-4 flex-shrink-0 text-red-400" />
                <p className="text-[10px] font-black uppercase tracking-wider table-text-muted">Pending review</p>
              </div>
              <div className="mt-2.5 flex min-h-[28px] items-center">
                <p className="text-2xl font-black leading-none text-white">{stats?.pendingGymReviewCount ?? 0}</p>
              </div>
            </div>
          </>
        )}
      ...
      <div className="dashboard-mobile-toolbar flex flex-wrap items-center justify-between gap-2 border-b table-border pb-4">
        ...
        <div className="dashboard-mobile-search relative max-w-[300px] flex-1">
          ...
          <input
            ...
            placeholder="Search titles, content, creators"
        ...
        <div className="dashboard-mobile-actions flex flex-shrink-0 flex-wrap items-center gap-2">
          ...
          <div ref={filterRef} className="relative">
            ...
            {filterOpen && (
              <div className="absolute right-0 top-[calc(100%+8px)] z-50 min-w-[260px] rounded-2xl border table-border table-bg p-4 shadow-[0_16px_48px_rgba(0,0,0,0.6)] space-y-3">
                <p className="text-[10px] font-black uppercase tracking-widest table-text-muted">Filters</p>

                <Select
                  value={sourceType ?? "ALL"}
                  onValueChange={(v) => {
                    setSourceType(v === "ALL" ? undefined : (v as AnnouncementSourceType));
                    setPage(0);
                  }}
                >
            ...
            {sortOpen && (
              <div className="absolute right-0 top-[calc(100%+8px)] z-50 min-w-[220px] rounded-2xl border table-border table-bg p-4 shadow-[0_16px_48px_rgba(0,0,0,0.6)] space-y-3">
                ...
                <Select
                  value={sort}
                  onValueChange={(v) => {
                    setSort(v as AdminAnnouncementSortMode);
                    setPage(0);
                    setSortOpen(false);
                  }}
                >
      ...
      <div className="overflow-hidden rounded-[18px] border table-border table-bg">
        <div className="dashboard-mobile-table-scroll">
          <table className="min-w-[1040px] w-full border-collapse" style={{ tableLayout: "fixed" }}>
          <thead>
            <tr className="table-header-bg border-b table-border">
              {["Announcement", "Source", "Publish", "Review", "Creator", "Scheduled", "Published", ""].map((h, i) => (
                <th
                  key={`${h}-${i}`}
                  style={{ width: COL_W[i] }}
            ...
            {listQuery.isLoading ? (
                ...
                <td colSpan={8} className="py-16 text-center">
                  ...
                  <div className="mt-1 text-[13px] table-text-muted">
                    {query
                      ...
                      : "Create the first admin announcement to get started"}
                  </div>
                </td>
              </tr>
            ) : (
              items.map((ann) => (
                ...
                >
                  ...
                  <td className="px-3.5 py-3.5" style={{ width: COL_W[3] }}>
                  ...
                  <td className="px-2 py-3.5 text-right" style={{ width: COL_W[7] }}>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48 border-white/10 bg-[#0f0f0f] text-white">
                        <DropdownMenuItem
                        ...
                        {canPublish(ann) && (
                          <DropdownMenuItem
                            className="cursor-pointer focus:bg-white/10 focus:text-white"
                            onClick={() => publishMutation.mutate(ann.announcementId)}
                            disabled={publishMutation.isPending}
                          >
                            <Send className="mr-2 h-4 w-4" /> Publish
                          </DropdownMenuItem>
                        )}
                        {ann.sourceType === "GYM" && ann.reviewStatus === "PENDING" && (
                          <>
                            <DropdownMenuSeparator className="bg-white/10" />
                            <DropdownMenuItem
                              className="cursor-pointer focus:bg-white/10 focus:text-white"
                              onClick={() => approveMutation.mutate(ann.announcementId)}
                        ...
                        {ann.publishStatus !== "PUBLISHED" && ann.publishStatus !== "EXPIRED" && (
                          <>
                            <DropdownMenuSeparator className="bg-white/10" />
                            <DropdownMenuItem
                              className="cursor-pointer table-text-muted focus:bg-white/10 focus:text-white"
                              onClick={() => cancelMutation.mutate(ann.announcementId)}
      ...
      <RejectAnnouncementDialog
        ...
        onSubmit={() =>
          ...
          rejectMutation.mutate({
            announcementId: rejectTarget.announcementId,
        ...
        <AlertDialogContent className="border-white/10 bg-[#0f0f0f] text-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/55">
              This removes the draft or scheduled admin announcement permanently. Published announcements
              cannot be deleted.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={(event) => {
                ...
                if (deleteTarget) deleteMutation.mutate(deleteTarget.announcementId);
              }}
