...
function tooltipContent(gym: GymRecommendationItem): string {
  const distance = formatDistance(gym.distanceMeters);
  const status = gym.currentlyOpen ? "Open" : "Closed";
  const occupancy = gym.occupancyPercent !== undefined ? ` | ${gym.occupancyPercent}% full` : "";

  return [
    `<div style="font-weight:800;text-transform:uppercase;font-size:12px;margin-bottom:3px">${gym.name}</div>`,
...
const GymsMapCanvas = ({
  ...
  useEffect(() => {
    ...
    const currentIds = new Set(filteredGyms.map((gym) => gym.id));
    ...
    filteredGyms.forEach((gym) => {
      ...
      const existingMarker = existing.get(gym.id);
