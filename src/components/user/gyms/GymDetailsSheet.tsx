...
import type { GymRecommendationItem, SheetSnap } from "./gyms.types";
...
const GymDetailsSheet = ({
  ...
  return (
    <div className="fixed left-0 right-0 z-[1001] mx-3 overflow-hidden transition-all duration-300 md:hidden bottom-[calc(var(--mobile-bottom-dock-height,80px)+env(safe-area-inset-bottom)+12px)]">
      <div
      ...
      >
        ...
        <div className="custom-scroll overflow-y-auto px-5 pb-5">
          <SheetContent
            ...
            onViewProfile={() => navigate(`/gym/${gym.id}`)}
            onCheckIn={() => onCheckIn(gym.id)}
