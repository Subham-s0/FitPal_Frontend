...
import DesktopSelectedGymCard from "./DesktopSelectedGymCard";
import EnableLocationDialog from "./EnableLocationDialog";
import GymsFilterDropdown from "./GymsFilterDropdown";
import GymDetailsSheet from "./GymDetailsSheet";
import GymsMapCanvas from "./GymsMapCanvas";
import GymsSidebarRail from "./GymsSidebarRail";
import RecommendationModeBar from "./RecommendationModeBar";
import { useGymsRecommendation } from "./useGymsRecommendation";

interface GymsSectionProps {
  onSwitchToCheckIn: (gymId?: string) => void;
}
...
const GymsSection = ({ onSwitchToCheckIn }: GymsSectionProps) => {
  const state = useGymsRecommendation();
  const navigate = useNavigate();
  const [currentPage, setCurrentPage] = useState(1);
  const locationEnabled = state.locationPermission === "granted" && !!state.userCoords;
  const selectedGymSaved = state.selectedGym ? state.savedGymIds.includes(state.selectedGym.id) : false;
  ...
  useEffect(() => {
    ...
    const selectedIndex = state.filteredGyms.findIndex((gym) => gym.id === state.selectedGym?.id);
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col overflow-hidden md:flex-row">
      <GymsSidebarRail
        ...
        selectedGymId={state.selectedGym?.id}
        onSelectGym={state.selectGym}
        locationEnabled={locationEnabled}
        savedGymIds={state.savedGymIds}
      ...
      <div className="relative flex-1 overflow-hidden bg-[#0a0a0a]">
        <div className="absolute left-0 right-0 top-0 z-[1000] space-y-2 p-4 pt-4 md:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <GymsFilterDropdown
              ...
              sortMode={state.sortMode}
              showSavedOnly={state.showSavedOnly}
              savedGymCount={state.savedGymIds.length}
              onStatusFilterChange={state.setStatusFilter}
              onSortModeChange={state.setSortMode}
              onToggleSavedOnly={state.toggleSavedOnly}
              onResetFilters={state.resetFilters}
          ...
          {!locationEnabled && (
            <div className="rounded-xl border border-white/10 bg-[rgba(10,10,10,0.84)] px-3 py-2 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-300 backdrop-blur-2xl">
        ...
        <DesktopSelectedGymCard
          ...
          onViewProfile={() => state.selectedGym && navigate(`/gym/${state.selectedGym.id}`)}
          onCheckIn={() => state.selectedGym && onSwitchToCheckIn(state.selectedGym.id)}
          onToggleSaved={() => state.selectedGym && state.toggleSavedGym(state.selectedGym.id)}
        />

        {!state.selectedGym && (
          <div className="absolute left-0 right-0 z-[1000] pb-4 pt-10 md:hidden bottom-[calc(var(--mobile-bottom-dock-height,52px)+env(safe-area-inset-bottom)+8px)]">
            <div className="flex gap-3 overflow-x-auto px-4 pb-2" style={{ scrollbarWidth: "none" }}>
              {pagedGyms.map((gym) => (
                <MobileGymCard
                  key={gym.id}
                  gym={gym}
                  isSaved={state.savedGymIds.includes(gym.id)}
                  onClick={() => state.selectGym(gym)}
                  onToggleSaved={() => state.toggleSavedGym(gym.id)}
                />
              ))}
              {totalPages > 1 && (
      ...
      <EnableLocationDialog
        ...
        onEnableLocation={state.requestLocation}
      />
    </div>
  );
};
    ...
    >
      <div className="mb-2 flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <h4 className="truncate text-xs font-extrabold uppercase tracking-tight">{gym.name}</h4>
      ...
      <div className="flex items-center gap-2 text-[9px] font-bold">
        <span className="text-orange-500">{formatDistance(gym.distanceMeters)}</span>
